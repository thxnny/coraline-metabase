# Sensitive values generated by Terraform and stored in Secrets Manager so they
# are injected into the ECS task at runtime rather than baked into the task def.
...
# Metabase uses this to encrypt saved data-source credentials & secrets at rest.
# Changing it later makes previously-saved secrets unreadable — keep it stable.
